...
class VendorService {
          ...
          if (error.response?.status >= 400 && error.response?.status < 500) {
            return {
              ...
              message:
                ...
                'Failed to fetch your vendor ads',
              data: [],
            };
          }
        ...
        try {
          ...
          if (error.response?.status >= 400 && error.response?.status < 500) {
            return {
              success: false,
              message:
                error.response?.data?.message || 'Failed to fetch vendor ads',
              data: [],
            };
          }
  ...
  async createVendorAd(vendorData, onUploadProgress = null) {
    try {
      if (vendorData instanceof FormData) {
        const result = await authUpload(
          `${API_BASE_URL}/vendor_ad`,
          vendorData,
          {
  ...
  formatVendorForDisplay(vendor) {
    ...
    if (photos.length === 0 && vendor.attachments) {
      ...
      if (
        typeof vendor.attachments === 'string' &&
        vendor.attachments !== '[]'
      ) {
        try {
    ...
    // Get category name
    let categoryName =
      vendor.category?.name || vendor.category_name || vendor.type || 'Service';
    ...
    const normalizeProfilePic = raw => {
      ...
      if (raw.startsWith('http://api.evnzo.com'))
    ...
    return {
      ...
      rating: Number.isFinite(Number(vendor.rating))
