...
class VendorDetailsService {
  ...
  async getUserReviews(userId, page = 1, limit = 10) {
    try {
      const response = await api.get(
        `/vendor-enhanced/user/${userId}/reviews`,
        {
          params: {page, limit},
        },
      );
  ...
  async toggleSaveVendor(vendorId, vendorData = {}) {
    try {
      ...
      const localResult = await savedVendorsStorage.toggleSaveVendor(
        itemId,
        vendorData,
      );
      ...
      try {
        ...
        const isSaved =
          response.data.data?.saved !== undefined
        ...
        const message =
          ...
          (isSaved ? 'Vendor saved' : 'Vendor unsaved');
...
👉 Open it on Evnzo: ${link}
  ...
  async getVendorAvailability(vendorId, startDate, endDate) {
    try {
      ...
      const response = await api.get(
        `/vendor-enhanced/${vendorId}/availability`,
        {params},
      );
      return {
        success: true,
  ...
  async getSimilarVendors(vendorId) {
    try {
      ...
      return {
        success: false,
        message:
          error.response?.data?.message || 'Failed to fetch similar vendors',
        data: [],
      };
    }
  }
      ...
      try {
        ...
        if (response.data.status && response.data.data) {
          ...
          const isBackendSaved = savedItems.some(item => {
            const itemId =
              item.item_id || item.itemId || item.item?.vendor_ad_id;
        ...
        const isLocallySaved = await savedVendorsStorage.isVendorSaved(
          vendorIdNum,
        );
