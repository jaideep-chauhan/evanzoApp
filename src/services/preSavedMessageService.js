...
class PreSavedMessageService {
    ...
    try {
      ...
      return {
        ...
        message:
          ...
          'Failed to retrieve pre-saved message',
        data: null,
      };
    }
  }
  ...
  async savePreSavedMessage(messageData) {
    try {
      const response = await api.post(
        '/vendor-enhanced/messages/pre-saved',
        messageData,
      );
      return {
        success: true,
        data: response.data.data,
        message:
          response.data.message || 'Pre-saved message saved successfully',
      };
    } catch (error) {
      return {
        success: false,
        message:
          error.response?.data?.message || 'Failed to save pre-saved message',
      };
    }
  }
  ...
  async updatePreSavedMessage(messageId, messageData) {
    try {
      const response = await api.put(
        `/vendor-enhanced/messages/pre-saved/${messageId}`,
        messageData,
      );
      return {
        success: true,
        data: response.data.data,
        message:
          response.data.message || 'Pre-saved message updated successfully',
      };
    } catch (error) {
      return {
        success: false,
        message:
          error.response?.data?.message || 'Failed to update pre-saved message',
      };
    }
  }
  ...
  async usePreSavedMessage(messageId) {
    try {
      const response = await api.post(
        `/vendor-enhanced/messages/pre-saved/${messageId}/use`,
      );
      return {
        success: true,
        data: response.data.data,
      ...
      return {
        success: false,
        message:
          error.response?.data?.message || 'Failed to use pre-saved message',
      };
    }
  }
  ...
  generateMessageTemplate(formData) {
    const {eventName, location, category, date, time, duration, description} =
      formData;
