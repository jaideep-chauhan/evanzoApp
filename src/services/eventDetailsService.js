      ...
      try {
        ...
        if (response.data.status && response.data.data) {
          ...
          const isSaved = savedItems.some(item => {
            const itemId =
              item.item_id || item.itemId || item.event_ad_id || item.id;
            const itemIdNum = parseInt(itemId, 10);
            console.log(
              `  Comparing saved item ID ${itemIdNum} with target ${eventIdNum}`,
            );
        ...
        const isLocallySaved = await savedEventsStorage.isEventSaved(
          eventIdNum,
        );
