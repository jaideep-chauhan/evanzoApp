...
class CategoryService {
  ...
  async getVendorCategories(includeSubcategories = true) {
    try {
      ...
      if (
        ...
        this.vendorCategoriesCache &&
        this.isCacheValid()
      ) {
        // Check if cached data has subcategories
        const hasSubcategoriesInCache = this.vendorCategoriesCache.some(
        ...
        console.log(
          '📦 Cache exists but lacks subcategories, fetching fresh data',
        );
      }
  ...
  async getEventCategories(includeSubcategories = true) {
    try {
      ...
      if (
        !includeSubcategories &&
        this.eventCategoriesCache &&
        this.isCacheValid()
      ) {
        // Check if cached data has subcategories
        const hasSubcategoriesInCache = this.eventCategoriesCache.some(
          cat => cat.subcategories && cat.subcategories.length > 0,
        );
      ...
      console.log('📦 Gig categories API response:', {
        status: response.data?.status,
        count: response.data?.data?.length,
        firstCategory: response.data?.data?.[0],
        hasSubcategories: response.data?.data?.[0]?.subcategories?.length > 0,
  ...
  getFallbackVendorCategories() {
    const fallbackCategories = [
      ...
      {category_id: 3, id: 3, name: 'Gig Decoration', slug: 'event-decoration'},
      ...
      {category_id: 7, id: 7, name: 'Gig Planning', slug: 'event-planning'},
