...
import Gigs from '../screens/events';
...
// instead of the actual screen content. Vendors / Gigs stay public.
const ProtectedChatList = withProtectedScreen(
  ChatList,
  'Sign in to view your messages',
);
const ProtectedProfile = withProtectedScreen(
  Profile,
...
function CustomTabBar({state, descriptors, navigation}) {
  ...
  const bottomGap =
    Platform.OS === 'android' ? Math.max(Math.min(insets.bottom, 16), 8) : 20;
